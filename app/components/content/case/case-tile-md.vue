  ...
  <div class="flex flex-col shrink-0">
    <CustomLink class="group hover:text-green-main flex flex-col w-full overflow-hidden" :disabled="itemData.draft && !itemData.fallback"
      ...
      <div ref="imgContainer"
        ...
        <div v-if="!itemData.draft || itemData.fallback"
        ...
        <nuxt-picture v-if="logoUrl" class="absolute z-10 top-6 left-6 md:top-12 md:left-12 grayscale brightness-0 "
          loading="lazy"
          :imgAttrs="{ class: 'w-auto max-h-12 max-w-[150px] md:max-w-[210px] object-cover object-left-top', wdith: 2100, height: 48 }"
          :src="logoUrl" :title="itemData.logo?.title ? itemData.logo.title : ''"
          :alt="itemData.logo?.alt ? itemData.logo.alt : 'Case study logo'" />
        <nuxt-picture v-if="imageUrl" class="relative z-0 h-full w-full group-hover:scale-105 will-change-transform transition duration-300 "
          loading="lazy"
          :imgAttrs="{ class: 'w-full h-full object-cover object-left-top', wdith: 665, height: 738 }"
          :src="imageUrl" :title="itemData.image?.title ? itemData.image.title : ''"
          :alt="itemData.image?.alt ? itemData.image.alt : 'Case study image'" />
      </div>
      <ul v-if="itemData.type" class="flex flex-wrap mb-5 desktop:mb-8 gap-x-5 gap-y-2.5">
        <li v-for="tag, index in itemData.type" :key="index"
          class="rounded-full text-gray-darker bg-gray-light px-5 py-2 text-xs desktop:text-sm font-medium uppercase">
          {{ tag }}</li>
      </ul>
      <h3 v-if="itemData.introduction?.title"
    ...
    <ul v-if="itemData.technologies && showTech" class="flex gap-[5px] h-[30px] mb-5 tablet:mb-8">
      <li v-for="tech, index in itemData.technologies" :key="index"
...
const imageUrl = computed(() => {
  return itemData.value.image?.url || itemData.value['image.url'] || ''
})

const logoUrl = computed(() => {
  return itemData.value.logo?.url || itemData.value['logo.url'] || ''
})

const hoverArrowPosition = computed(() => {
  if (!imgContainer.value || !props.mousePos?.x?.value || !props.mousePos?.y?.value) return {};
