<template>
  ...
  >
    <EffectAppearMdc class="desktop-delay h-full" :delay="delay * 100">
      ...
      >
        <div
          class="group h-full border-2 bg-white border-gray-light hover:border-green-main rounded-[5px] transition duration-200"
        >
          <CustomLink
            :url="link"
            :activeClass="'none'"
          ...
          >
            <nuxt-picture
              v-if="icon"
              class="shrink-0"
              :class="{ 'mb-5': $slots.description || stylesMinimal },stylesCustom ? 'p-2.5 rounded-md bg-green-light':'mr-10'"
              :imgAttrs="{ class: 'w-10 h-10 object-contain' + (stylesCustom ? ' md:w-14 md:h-14':'') }"
