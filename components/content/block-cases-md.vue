<template>
  <div v-if="contentFolder">
    ...
    <EffectAppearMd>
      <div v-if="moreBtnVisible" class="!relative mt-10 desktop:mt-12">
        <CustomLink
          :url="moreBtn.url"
...
const casesFiltered = computed(() => {
  ...
  if (props.selectedSlugs.length > 0) {
    ...
    casesArray.sort(
      (a, b) => selectedPaths.indexOf(a._path) - selectedPaths.indexOf(b._path)
    );
  }
  return casesArray;
});
...
function checkCaseVisibility(case_page) {
  return (
    checkTechFilters(case_page.technologies) && checkSelectedSlugs(case_page)
  );
}
