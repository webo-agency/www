<template>
  <div>
    <ul
      class="flex justify-between flex-wrap desktop:flex-nowrap gap-2.5 tablet:gap-5 mb-10 tablet:mb-[100px]"
    >
      <li
        v-for="(tab, index) in tabNames"
        :key="index"
        class="basis-full tablet:basis-[calc(50%_-_10px)] desktop:basis-[calc(33.3%_-_13.3px)] flex items-center p-5 desktop:p-7 border-2 transition duration-300 rounded-[5px] cursor-pointer"
        :class="[
          activeTabId == index
            ? 'border-green-main hover:border-green-main'
            : 'border-gray-default hover:border-green-mainHover/40',
        ]"
      ...
      >
        <nuxt-picture
          v-if="tab.iconUrl"
          :src="tab.iconUrl"
          class="h-10 tablet:h-[60px] mr-3 tablet:mr-5"
          :img-attrs="{ class: 'h-full object-contain ' }"
        >
        </nuxt-picture>
        <span class="text-lg tablet:text-xl desktop:text-2xl font-semibold">
          {{ tab.name }}
        </span>
      </li>
    </ul>
    <div class="px-5 tablet:px-10 desktop:px-[50px]">
        <!-- <div class="w-fyll relative">
            <div v-for="(tab, index) in tabNames" :key="index">
              <Transition>
                <div class="w-full" v-show="activeTabId == index">
                  <slot :name="tab.name"></slot>
                </div>
              </Transition>
            </div>
            <div class="hidden desktop:block absolute top-10 -right-32">
              <svg
                width="96"
                height="128"
                viewBox="0 0 96 128"
