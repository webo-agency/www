        ...
        <div v-if="!isSidebar || (isSidebar && socials?.length)" :class="[isSidebar ? '' : 'flex flex-wrap items-center']">
            <div v-if="isSidebar" class="bg-green-gray p-6 rounded-lg">
                ...
                <ul class="!mb-0 flex flex-wrap gap-3">
                    <SocialShare v-for="network in socials" :key="network" :network="network" :styled="true" :label="false" class="rounded-[5px] bg-green-light w-[46px] h-[46px] _p-2.5 p-0 [&_svg_*]:!fill-[#202020] [&_svg]:w-[35px] [&_svg]:h-[35px] hover:bg-green-main transition duration-200 flex items-center justify-center" />
                    <button v-if="showCopyLink" @click="copyLink" class="group rounded-[5px] bg-transparent w-[46px] h-[46px] _p-2.5 p-0 hover:bg-green-main transition duration-200 flex items-center justify-center">
                        <svg class="" xmlns="http://www.w3.org/2000/svg" width="29" height="29" viewBox="0 0 29 29" fill="none">
            ...
            <div v-else class="flex flex-wrap items-center">
                ...
                <ul class="flex flex-wrap gap-5">
                    <SocialShare v-for="network in socials" :key="network" :network="network" :styled="true" :label="false" class="rounded-[5px] _bg-gray-dark w-[30px] h-[30px] p-1 [&_svg_*]:!fill-[#202020]" />
                </ul>
            </div>
        </div>
...
:deep(.tags-list li a) {
  @apply bg-white border border-solid border-gray-default text-gray-darker rounded-full py-3 px-5 text-sm uppercase font-normal leading-none hover:text-white hover:bg-green-main hover:border-green-main transition duration-200 inline-block;
}
</style>