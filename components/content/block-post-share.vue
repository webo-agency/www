        ...
        <div v-if="!isSidebar || (isSidebar && socials?.length)" :class="[isSidebar ? '' : 'flex flex-wrap items-center']">
            <div v-if="isSidebar" class="bg-green-gray p-6 rounded-lg">
                ...
                <ul class="!mb-0 flex flex-wrap gap-3">
                    <SocialShare v-for="network in socials" :key="network" :network="network" :styled="true" :label="false" class="rounded-[5px] bg-green-light w-[46px] h-[46px] p-0 [&_svg_*]:!fill-[#202020] [&_svg]:w-[35px] [&_svg]:h-[35px] hover:bg-green-main transition duration-200 flex items-center justify-center" />
                    <button v-if="showCopyLink" @click="copyLink" class="group rounded-[5px] bg-transparent w-[46px] h-[46px] p-0 hover:bg-green-main transition duration-200 flex items-center justify-center">
