<template>
  <nav class="fixed top-0 left-0 w-full z-40" :class="{
    ...
    <div
      ...
      :class="{ 'opacity-0': !mobileExpanded || isLargeScreen }"></div>
      ...
      <div ref="container"
        ...
        <div class="flex items-center justify-between py-3 tablet:py-2 desktop:py-0">
          <CustomLink :url="'https://www.webo.agency/'" :activeClass="'none'" aria-label="Homepage" @click="
            ...
            ">
            <svg class="fill-current transition duration-300 h-[30px] tablet:h-[41px] w-auto" width="135" height="41"
              viewBox="0 0 135 41" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <HamburgerBtn class="flex flex-col justify-between desktop:hidden" :btnActive="mobileExpanded"
            @click="toggleMobile()" />
        </div>
        <div
          itemscope
  ...
  " @mouseleave="
    ...
    ">
              ...
              <div ref="collapsible" data-collapsed="true" class="relative transition duration-300" v-if="item.menu">
                <div
                  ...
                  <svg
                    ...
                    width="15" height="8" viewBox="0 0 10 6" xmlns="http://www.w3.org/2000/svg">
                    <path
            ...
            <li class="mb-2 desktop:mb-0 transition-[transform,opacity] duration-300 order-[9999]" :class="{
              '-translate-x-full desktop:translate-x-0 opacity-0':
                !mobileExpanded && !isLargeScreen,
            }" :style="[
  {
    ...
    order:
      isLargeScreen && menu.length > 0
        ...
        : '9999',
          ...
          <div class="desktop:hidden flex flex-col mb-10 transition duration-500" :class="{
            'translate-y-[80vh] desktop:translate-y-0 opacity-0':
              !mobileExpanded && !isLargeScreen,
          }">
            <Socials v-if="socials" class="mb-10" :socials="socials" :invertColors="true" />
            <WidgetRatingClutch :shrinkMobile="false" class="!p-0"
              :class="{ '!bg-green-gray': mobileExpanded && !isLargeScreen }" />
          </div>
        </div>
      </div>
      <div class="hidden desktop:block mx-auto max-w-screen-desktop-wide" @mouseover="popUpHover = true"
        @mouseleave="(popUpHover = false), setPopup(false)">
        <SubmenuPopup :active="popUpActive" :name="hoverItem" :data="popUpData" :pointerPosition="pointerPosition"
