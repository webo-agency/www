---
meta:
    title: Thoro - Wdrożenie nowej szaty graficznej sklepu internetowego Prestashop, polskiego producenta oświetlenia.
    description: 'Wdrożenie redesignu sklepu internetowego Prestashop, w oparciu o przekazany projekt UX/UI i istniejącą infrastrukturę sklepu.'  
introduction.title: 'Thoro - Wsparcie i rozwój sklepu internetowego Prestashop polskiego producenta oświetlenia'
introduction.description: 'Thoro jest producentem oraz autorską marką oświetlenia, łączącą doskonałe wzornictwo oraz trwałą jakość. Nasza współpraca polega na rozwoju sklepu o nowe funkcjonalności, wsparciu w przypadku awarii, optymalizacji technologicznej oraz projektowaniu i wdrażaniu nowych podstron.'
type:
    - UX/UI
    - E-commerce
    - Wsparcie/Rozwój
technologies:
    - PrestaShop
image.url: '/img/case-studies/Thoro-bg.png'
logo.url: '/img/logos/thoro.svg'
tile_hidden: false
tile_order: 1
public: true
draft: false
---


::page-section-md
---
class: mb-10 tablet:mb-20 mt-[100px] tablet:mt-[200px]
---
:::effect-appear-md
::::block-header-case-md
---
class: mx-default
logoUrl: '/img/logos/thoro.svg'
link: 'https://thoro.pl/'
---

#title
## Thoro - Wdrożenie nowej szaty graficznej sklepu internetowego Prestashop, polskiego producenta oświetlenia.

#description
**Thoro** jest producentem oświetlenia, które łączy minimalistyczny design z najwyższą jakością wykonania. Naszym zadaniem było **wdrożenie redesignu sklepu internetowego Prestashop**, w oparciu o przekazany projekt UX/UI i istniejącą infrastrukturę sklepu.

#buttontext
Zobacz online

#tags
- Sklep internetowy
- Programowanie

::::
:::
::

::page-section-md
---
class: mb-[50px] tablet:mb-[100px]
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-thoro-img1.webp"}
::::
:::
::


::page-section-md
---
class: _static-content mb-[50px] tablet:mb-[100px]
---

:::effect-appear-md
---
class: _mx-default
---
::::block-columns-1-1-md
---
reverseMobile: true
class: items-center [&_div.right]:self-start
---

#left
::::card-icon-md
---
class: tablet:pr-2.5 [&_div_div.group]:p-10 [&_div_div]:bg-primary [&_div_div]:!text-gray-darker [&_h4]:!mb-4 [&_h4]:tablet:text-[35px]
icon: '/img/icons/info.svg'
---
#title
#### Projekt w pigułce

#content
:::::block-list-ul-md
---
decoration: false
---
::::::list-item-ul-active-md
---
iconUrl: '/img/icons/list-1.svg'
iconBig: false
animated: false
class: icon [&_div_div]:!pb-1 [&_div_div_div]:!min-h-fit [&_div_div.flex]:lg:flex-row [&_div_div.flex]:gap-7 [&_div_div_picture]:!w-[60px] [&_div.title_p]:!text-xl [&_div.title]:!mb-2 [&_div_div_div_div_p]:!tracking-normal

---

#title
Przygotowanie środowiska deweloperskiego

#description
Przygotowaliśmy środowisko deweloperskie wraz z **repozytorium projektu na GitHub**, dzięki czemu zyskaliśmy pełną kontrolę nad kodem i możliwość **bezpiecznej pracy nad nowym layoutem**. Pozwoliło to wdrażać zmiany bez wpływu na ciągłość sprzedaży sklepu produkcyjnego.

::::::
::::::list-item-ul-active-md
---
iconUrl: '/img/icons/list-2.svg'
iconBig: false
animated: false
class: icon [&_div_div]:!pb-1 [&_div_div_div]:!min-h-fit [&_div_div.flex]:lg:flex-row [&_div_div.flex]:gap-7 [&_div_div_picture]:!w-[60px] [&_div.title_p]:!text-xl [&_div.title]:!mb-2 [&_div_div_div_div_p]:!tracking-normal
---

#title
Wdrożenie nowej szaty graficznej sklepu

#description
Nowy wygląd sklepu Thoro zakodowaliśmy na podstawie **projektu dostarczonego przez klienta**. Wymagało to precyzyjnego odzworowania widoków desktop i mobile, wraz z **dostosowaniem funkcjonalności do nowych założeń** - tak aby całość była spójna i funkcjonalna.

::::::
::::::list-item-ul-active-md
---
iconUrl: '/img/icons/list-3.svg'
iconBig: false
animated: false
class: icon [&_div_div]:!pb-1 [&_div_div_div]:!min-h-fit [&_div_div.flex]:lg:flex-row [&_div_div.flex]:gap-7 [&_div_div_picture]:!w-[60px] [&_div.title_p]:!text-xl [&_div.title]:!mb-1 [&_div_div_div_div_p]:!tracking-normal
---

#title
Testy i publikacja

#description
Przeprowadziliśmy **szczegółowe testy funkcjonalne i wizualne**, weryfikując poprawność wdrożenia na wszystkich etapach zakupowych. **Po zatwierdzeniu jakości opublikowaliśmy nowy wygląd sklepu** na środowisku produkcyjnym.

::::::
:::::
::::

#right

::::card-icon-md
---
class: tablet:pr-2.5 [&_div_div.group]:p-10 [&_div_div]:bg-white [&_div_div]:!text-gray-darker [&_h4]:!mb-4 [&_h4]:tablet:text-[35px]
icon: '/img/icons/double_check.svg'
---
#title
#### Rezultat pracy i dalsze wsparcie

#content
:::::block-list-ul-md
---
decoration: false
---
::::::list-item-ul-active-md
---

iconBig: false
animated: false
class: icon [&_div_div_div]:!min-h-fit 
---

#description
Efektem naszej pracy jest **nowy wygląd sklepu internetowego PrestaShop** naszego klienta, który kompleksowo odświeżył doświadczenie zakupowe użytkowników. **Zaprogramowaliśmy ponad 40 widoków UX/UI** przygotowanych wcześniej przez agencję brandingową - zarówno w wersji desktop, jak i mobile, **pracując na istniejącej infrastrukturze sklepu** (Backend). Projekt obejmował m.in. wdrożenie strony głównej, kategorii, karty produktowej, koszyka i procesu zakupowego, a także podstron takich jak „O nas”, „Dystrybutorzy”, „Projektanci” i wiele innych. Dzięki temu sklep Thoro zyskał **nowoczesny i spójny interfejs**, w pełni zgodny z nowym brandingiem marki, **bez zmiany w działaniu back office** (panelu administracyjnego).<br><br>
W ramach dalszej współpracy sprawujemy **stałą opiekę nad sklepem**, dbając o **stabilność działania, bieżące wsparcie** oraz rozwijając go o **nowe funkcjonalności i podstrony** zgodnie z potrzebami naszego klienta.

::::::
:::::


::::
:::
::



::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-thoro-img2.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-thoro-img3.webp"}
::::
:::
::

::page-section-md
---
class:
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-thoro-img4.webp"}
::::
:::
::

::page-section-md
---
class:
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-thoro-img5.webp"}
::::
:::
::

::page-section-md
---
class: mb-5
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-thoro-img6.webp"}
::::
:::
::

::page-section-md
---
class:
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-thoro-img7.webp"}
::::
:::
::

::page-section-md
---
class:
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-thoro-img8.webp"}
::::
:::
::

::page-section-md
---
class:
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-thoro-img9.webp"}
::::
:::
::

::page-section-md
---
class: mb-5 tablet:mb-[100px]
---
:::effect-appear-md
::::prose-figure{src="/img/content-images/case-thoro-img10.webp"}
::::
:::
::




::page-section-md
---
class: mb-[50px] tablet:mb-[100px]
---
:::effect-appear-md
::::block-cta-md
---
button.link: https://webo.agency/kontakt
button.title: 'Skontaktuj się'
---

#title
#### Potrzebujesz ekspertów do realizacji *strony internetowej?*

#description
Umów się na bezpłatną konsultację, porozmawiajmy!

::::
:::
::

::page-section-md
---
id: realizacje
class: mb-[80px] tablet:mb-[130px]
---
:::effect-appear-md
::::block-header-md
---
class: mb-10 tablet:mb-20
---

#subtitle
### *2/* Nasze ostatnie realizacje

#title
## Zobacz nasze pozostałe *realizacje stron www*

::::
:::

:::block-cases-md
---
contentFolder: realizacje
moreBtn.url: 'https://webo.agency/realizacje'
moreBtn.linktitle: Zobacz wszystkie projekty
selectedSlugs:
    - isw
    - mutiny-group
---

:::
::

::page-section-md
---
class: mb-20 tablet:mb-[130px]
---
:::effect-appear-md
::::block-header-md
---
center: true
class: mb-[50px] tablet:mb-[100px]
---

#subtitle
### *3/* Nasi klienci

#title
## Poznaj firmy które *nam zaufały*

::::
:::
:::block-brands-before-footer-md
:::
::
